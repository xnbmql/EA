...
public:
   ...
   int               PennigSellWithPrice(double price, double volume,int stopLossPoint=0, int takeProfitPoint=0,datetime expiration=0,string commento=NULL,color arrowColoro=clrNONE);
   int               PennigBuyWithPrice(double price, double volume,int stopLossPoint=0, int takeProfitPoint=0,datetime expiration=0,string commento=NULL,color arrowColoro=clrNONE);
   ...
   int               SendOrder(int cmd, double volume, double price, int slippageo, double stoploss, double takeprofit, string commento=NULL, datetime expiration=0, color arrow_coloro=clrNONE,bool stopTrace=false);
   ...
   int               SendWithStAndTpOrderByPrice(int operation, double volume,double stopLossPrice=0, double takeProfitPrice=0,datetime expiration=0,string commento=NULL,color arrowColoro=clrNONE,bool stopTrace=false);
...
private:
   ...
   int               getPenningType(int baseOperate, double price);
...
//|                                                                  |
//+------------------------------------------------------------------+
int OrderManager::SendWithStAndTpOrderByPrice(int operation, double volume,double stopLossPrice=0, double takeProfitPrice=0,datetime expiration=0,string commento=NULL,color arrowColoro=clrNONE,bool stopTrace=false)
  {
   RefreshRates();
   double basePrice = getBasePriceByOrderType(operation);
   Print("basePrice:",basePrice,"sl:",stopLossPrice,"tp:",takeProfitPrice);
//double sl = calculateStopLossByPoint(getStopBasePriceByOrderType(operation),operation,stopLossProfitPoint);
//double tp = calculateTakeProfitByPoint(getStopBasePriceByOrderType(operation),operation,takeProfitPoint);
   return SendOrder(operation,volume,basePrice,slippage,stopLossPrice,takeProfitPrice,commento,expiration,arrowColoro,stopTrace);
  }
...
// 跟当前价格点差不能是0 差值（绝对值）要大于 Slippage
   if(pricePointDiff == 0 || MathAbs(pricePointDiff)<=slippage || MathAbs(pricePointDiff)< MarketInfo(symbol,MODE_SPREAD))
     {
...
//|                                                                  |
//+------------------------------------------------------------------+
int OrderManager::PennigBuyWithPrice(double price, double volume,int stopLossPoint=0, int takeProfitPoint=0,datetime expiration=0,string commento=NULL,color arrowColoro=clrNONE)
  {
   int type = getPenningType(OP_BUY, price);
   if(type < 0)
     {
      return 0;
     }
...
//|                                                                  |
//+------------------------------------------------------------------+
int OrderManager::PennigSellWithPrice(double price, double volume,int stopLossPoint=0, int takeProfitPoint=0,datetime expiration=0,string commento=NULL,color arrowColoro=clrNONE)
  {
   int type = getPenningType(OP_SELL, price);
   if(type < 0)
     {
      return 0;
     }
...
//|                                                                  |
//+------------------------------------------------------------------+
int OrderManager::getPenningType(int baseOperate, double price)
  {
   int mod = 0;
   if(baseOperate == OP_BUY)
     {
      mod = MODE_ASK;
     }
   else
   ...
   if(MathAbs(price-MarketInfo(symbol,mod)) < MarketInfo(symbol,MODE_SPREAD)*MarketInfo(symbol,MODE_POINT))
     {
      Print("Send Penning get wrong Price, marketPrice: ",MarketInfo(symbol,mod), " intputPrice: ", price, "  spreadPrice: ",MarketInfo(symbol,MODE_SPREAD)*MarketInfo(symbol,MODE_POINT));
      return -1;
     }
   switch(baseOperate)
     {
      case OP_BUY:
         if(price > MarketInfo(symbol,mod))
           {
            return OP_BUYSTOP;
           }
         else
...
//|                                                                  |
//+------------------------------------------------------------------+
int OrderManager::SendOrder(int cmd, double volume, double price, int slippageo, double stoploss, double takeprofit,string commento=NULL, datetime expiration=0, color arrow_coloro=clrNONE,bool stopTrace=false)
  {
